...
1. **Blender speichern** (über BlenderMCP): `bpy.data.filepath` und `bpy.data.is_dirty` auslesen.
   ...
   - **Ist im Projektwerte-Block keine Arbeitsdatei eingetragen** (Stand 2026-07-22 der Fall): nicht speichern, sondern Dominik fragen, welche Datei die Arbeitsdatei ist, und den Eintrag ergänzen.
   - Ist `filepath` eine **andere** Datei (Referenz, leer, falsches Fenster): **nicht speichern**, Dominik warnen, dass die falsche Datei geladen ist.
...
3. **Restliche Doku abgleichen:** neue Entscheidungen, neue oder behobene Probleme, Asset-/Textur-Änderungen, offene Punkte. Welche Dateien es dafür gibt, steht in der `CLAUDE.md` unter „Dokumentation" — **existiert eine davon noch nicht, erst mit Dominik klären, statt sie blind anzulegen.**
